/**
 * PackyAPI 平台适配器
 * 使用与 NewAPI 相同的 API 接口（One API 兼容）
 */
 ...
 * PackyAPI 平台适配器
 */
export class PackyAPIAdapter extends BasePlatformAdapter {
    ...
    async fetchQuota(credentials: PlatformCredentials): Promise<PlatformQuotaResult> {
        try {
            const response = await this.httpRequest<PackyAPIResponse>(
